...
import stomp
import sys, io, os, urllib, json
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from NeoHoney_Analyse import NeoHoneyAnalyserPluginInterface

class AnalysePlugin(NeoHoneyAnalyserPluginInterface):
    _ExtraRuleFields = {
        "ContentBody": (
    ...
    def _AnalyseSingleData(self, InputData, InputRule):
        '数据分析方法接口，接收被分析的dict()类型数据和命中的规则作为参考数据，应返回True/False'
        try:
            if self.conn == None:
